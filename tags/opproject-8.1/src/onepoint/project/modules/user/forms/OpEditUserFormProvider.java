...
public class OpEditUserFormProvider implements XFormProvider {
   ...
   private static final XLog logger = XLogFactory.getLogger(OpEditUserFormProvider.class);
   ...
   private XComponent fillLanguageDataSet(XComponent form, OpProjectSession session, OpBroker broker, OpUser user) {
      ...
      OpUserLanguageManager.fillLanguageDataSet(session, languageDataSet, languageField, currentUser);

      //get the user preferences. If the user has no preference, get the preference from system setings.
      String preference;
      if(user.getPreference(OpPreference.LOCALE_ID) != null){
         preference = user.getPreference(OpPreference.LOCALE_ID).getValue();
      }
      else{
         preference = OpSettingsService.getService().getStringValue(session, OpSettings.USER_LOCALE_ID);
      }
