...
public class OpMailer implements Runnable {
   ...
   private static final XLog logger = XLogFactory.getLogger(OpMailer.class);
   ...
   public void sendMessages(ArrayList messages) throws MessagingException {
      ...
      while (i.hasNext()) {
         ...
         mime_message.setSubject(message.getSubject(), "UTF-8");
         if (message instanceof OpHTMLMailMessage) {
            ...
            mime_message.setHeader("Content-Transfer-Encoding", "8bit" );
            mime_message.setHeader("Content-Type","text/plain;charset=utf-8"); 
            mime_message.setText(message.content(), "UTF-8");//OpMailMessage.TEXT_PLAIN);
         }
