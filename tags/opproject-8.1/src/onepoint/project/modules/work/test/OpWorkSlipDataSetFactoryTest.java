...
public class OpWorkSlipDataSetFactoryTest extends OpBaseOpenTestCase {
   ...
   public void testFormDataSetsFromWorkRecordsTwoWR()
      ...
      try {
         ...
         workRecord1.getAssignment().getActivity().addAssignment(assignment);
   ...
   */
   public void testFormWorkRecordsFromDataSetsNoWR()
      ...
      try {
         ...
         List<OpWorkRecord> workRecords = OpWorkSlipDataSetFactory.formWorkRecordsFromDataSets(broker, workRecordDataSet,
              timeRecordDataSet, costRecordDataSet, null);
...
//       broker.close();
   ...
   public void testFillProjectSet()
      ...
      String projectId = projectFactory.getProjectId(PROJECT_NAME);
      OpProjectNode project = projectFactory.getProjectById(projectId);
      OpProjectPlan projectPlan = projectFactory.getProjectPlanById(project.getPlan().locator());
      ...
      OpProjectPlan projetPlan1 = projectFactory.getProjectPlanById(project1.getPlan().locator());

      List<OpAssignment> assignmentList = new ArrayList<OpAssignment>();
      assertEquals(0, projectsSet.getChildCount());

      for (OpAssignment assignment : projectPlan.getActivityAssignments()) {
         assignmentList.add(assignment);
      }
      for (OpAssignment assignment : projetPlan1.getActivityAssignments()) {
         assignmentList.add(assignment);
      }
   ...
   public void testFillChoiceDataSetsFromSingleAssignment()
      ...
      try {
         String project1Id = projectFactory.getProjectId(PROJECT_NAME + 1);
         OpProjectNode project1 = projectFactory.getProjectById(project1Id);
         OpProjectPlan projectPlan1 = projectFactory.getProjectPlanById(project1.getPlan().locator());

         OpAssignment assignment = new OpAssignment();
         for (OpAssignment activityAssignment : projectPlan1.getActivityAssignments()) {
            activityAssignment = (OpAssignment) broker.getObject(activityAssignment.locator());
