...
public class OpMyTasksService extends OpProjectService {
   ...
   public XMessage addAdhocTask(OpProjectSession session, XMessage request) {
      ...
      try {
         ...
         adhocTaks.addAssignment(assignment);
         assignment.setResource(resource);
         assignment.setProjectPlan(adhocTaks.getProjectPlan());
         assignment.attachToActivity(adhocTaks);
   ...
   private void updateAttachments(OpProjectSession session, OpBroker broker, OpActivity activity, XComponent attachmentSet) {
      ...
      for (OpAttachmentIfc attachment : existingSet) {
         if (!attachmentsRowMap.keySet().contains(attachment.locator())) {
            serviceImpl.deleteAttachment(session, broker, (OpAttachment) attachment);
         }
         else {
            // check if the attachment had it's name updated
            XComponent attachmentRow = attachmentsRowMap.get(attachment.locator());
            String choice = ((XComponent) attachmentRow.getChild(1)).getStringValue();
            String caption = XValidator.choiceCaption(choice);
            if (!attachment.getName().equals(caption)) {
               attachment.setName(caption);
               broker.updateObject((OpAttachment) attachment);
            }
