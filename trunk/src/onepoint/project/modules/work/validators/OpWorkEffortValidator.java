...
//   public final static int DONE = 2;
   ...
   public final static int ACTIONS_STATUS = 13;
   ...
   private static final String NOT_ALL_ACTIONS_DONE_EXCEPTION = "NotAllActionsDone";
   ...
   private static final XLog logger = XLogFactory.getLogger(OpWorkEffortValidator.class);
      ...
      for (int i = 0; i < data_set.getChildCount(); i++) {
         ...
         if (getRemainingEffort(row) == null) {
            ...
            if (getRemainingEffort(row).doubleValue() < 0) {
               ...
               if (getRemainingEffort(row).doubleValue() == 0) {
                  if (!allActionsDone(row)) {
                     throw new XValidationException(
                           NOT_ALL_ACTIONS_DONE_EXCEPTION);                     
                  }
   ...
   private boolean allActionsDone(XComponent row) {
      switch (getActionsStatus(row)) {
      case OpGanttValidator.NO_ACTIONS:
      case OpGanttValidator.DONE:
         return true;
      }
      return false;
   }
      ...
      if (cell.getValue() == null || !cell.getValue().equals(value)) {
         switch (column_index) {
            ...
            case ACTUAL_EFFORT_INDEX:
               ...
               if (getPulsing() != null) {
                  int minutes = (int) (actualEffort * OpProjectCalendar.MINUTES_PER_HOUR);
                  int pulsing = getPulsing().intValue();
                  if (pulsing != 0 && minutes % pulsing != 0) {
                     minutes = (minutes / pulsing + 1) * pulsing;
                  }
                  actualEffort = (double) minutes / OpProjectCalendar.MINUTES_PER_HOUR;
               }
               ...
               if (activityChoice != null && resourceChoice != null &&
                  ...
                  if (remainingEffortCell.getEnabled()) {
                     ...
                     if (remainingEffortCell.getDoubleValue() + oldCellValue - cell.getDoubleValue() > 0) {
                        ...
                        if (!allActionsDone(dataRow)) {
                           throw new XValidationException(NOT_ALL_ACTIONS_DONE_EXCEPTION);                     
                        }
            ...
            case REMAINING_EFFORT_INDEX:
               ...
               if (remainingEffort == 0) {
                  ...
                  setValue(dataRow, COMPLETED_INDEX, new Boolean(true), true);
   ...
   public boolean getEnabled(XComponent row) {
      Boolean enabled = (Boolean) getValue(row, ENABLED_INDEX);
   ...
   public void updateEffortCells(String activityChoice, String resourceChoice, XComponent dataRow, boolean progressTracked, XComponent mapField) {
      ...
      if (((XComponent) dataRow.getChild(ACTIONS_STATUS)).getValue() != null) {
         Integer actionStatus = (Integer) assignmentDataList.get(OpWorkValidator.ACTIONS_STATUS);
         setValue(dataRow, ACTIONS_STATUS, actionStatus);
         ((XComponent)dataRow.getChild(COMPLETED_INDEX)).setEnabled(allActionsDone(dataRow) && progressTracked);
      }
   ...
   private static XComponent ensureChildCreated(XComponent data_row, int column) {
      int childCount = data_row.getChildCount();
      for (; column >= childCount; childCount++) {
         data_row.addChild(new XComponent(XComponent.DATA_CELL));
      }
      return ((XComponent)data_row.getChild(column));
   }

   // Helpers for easier accessing data-set values
   private static void setSomething(XComponent data_row, int column, Object value) {
      ensureChildCreated(data_row, column).setValue(value);
   }
   ...
   private static boolean columnHoldsValue(XComponent data_row, Class c,
         int column) {
      if (column < data_row.getChildCount()) {
         return ((XComponent) (data_row.getChild(column))).hasValue(c);
      }
      return false;
   }
    ...
    * @post
    */
   private static int getActionsStatus(XComponent action) {
      return columnHoldsValue(action,Integer.class, ACTIONS_STATUS) ? ((Integer) getSomething(action, ACTIONS_STATUS)).intValue()
            : 0;
   }
