...
public class OpWorkTimeValidator extends OpWorkValidator {
   ...
   public boolean validateDataSet() {
      ...
      for (int i = 0; i < data_set.getChildCount(); i++) {
         ...
         if (startMinutes < 0 || startMinutes > OpProjectCalendar.MINUTES_PER_DAY) {
            throw new XValidationException(START_VALUE_EXCEPTION);
         }
   ...
   private void updateFinish(XComponent dataRow) {
      ...
      if (startMinutes + durationMinutes < OpProjectCalendar.MINUTES_PER_DAY) {
         setValue(dataRow, FINISH_INDEX, new Integer(startMinutes + durationMinutes));
      }
      else {
         startMinutes = OpProjectCalendar.MINUTES_PER_DAY - 1 - durationMinutes;
   ...
   public void updateHoursDataSet() {
      ...
      for (int i = 0; i < hours.getChildCount(); i++) {
         ...
         if (locator != null) {
            ...
            if (newEffort != null) {
               ...
               Double actualEffort = new Double(newEffort.doubleValue() / OpProjectCalendar.MINUTES_PER_HOUR);
      ...
      for (Iterator iterator = efforts.entrySet().iterator(); iterator.hasNext();) {
         ...
         if (timeRow != null) {
            ...
            Double actualEffort = new Double(effort.doubleValue() / OpProjectCalendar.MINUTES_PER_HOUR);
