...
public class OpOpenApplet extends XExpressApplet {
   ...
   public void showStartForm(Map parameters) {
      ...
      // everytime we rceive an start form we update the one from client. E.g after the configuration wizard,
      // login page should be the start form (OPP-577)
      if (startForm != null) {
         getClient().setVariable(OpProjectConstants.START_FORM, startForm);
      }
   ...
   private boolean autoLogin() {
      ...
      if (value != null) {
         ...
         if (response.getError() == null) {
            OpProjectCalendar calendar = (OpProjectCalendar) client.getVariable(OpProjectConstants.CALENDAR);
