...
public class OpHibernateSource extends OpSource {
   ...
   /**
    * Returns an SQL statement that will upgrade the next_hi value used by the hibernate
    * hi/lo generation, when upgrading the entire db. This is needed in order to avoid duplicate
    * ids being generated by hibernate, as the initial value generated by Hibernate for this
    * field is always 0.
    *
    * @return a <code>String</code> representing an sql statement
    *         <FIXME author="Horia Chiorean" description="When multi-site functionality is finished, check if this is ok for multi-site as well, or an override is needed">
    * @see org.hibernate.id.TableHiLoGenerator
    */
   private String getUpdateHiLoGeneratorStatement(Session session) {
      SQLQuery query = session.createSQLQuery(MAX_OBJECT_ID_QUERY);
      List resultList = query.list();
      long maxId = !resultList.isEmpty() ? ((Number) resultList.get(0)).longValue() : 0;
      //this calculation is based on the implementation of the hibernate HiLo generator
      long nextHi = maxId / Short.MAX_VALUE + 1;
      return UPDATE_HILO_GENERATOR_STATEMENT + nextHi;
   }

   /**
    * Updates the hibernate hi/lo generator table in order to ensure a consistent value.
    */
   public void updateHiLoGeneratorValue() {
      Session session = sessionFactory.openSession();
      String statement = this.getUpdateHiLoGeneratorStatement(session);
      Transaction transaction = session.beginTransaction();
      SQLQuery sqlQuery = session.createSQLQuery(statement);
      int resut = sqlQuery.executeUpdate();
      transaction.commit();
      session.close();
      if (resut <= 0) {
         logger.error("Unable to update the hi/lo generator value");
      }
   }
