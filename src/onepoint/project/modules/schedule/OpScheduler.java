  ...
  private OpScheduler(){
   ...
   public static void registerSchedule(OpProjectSession session, OpSchedule schedule) {
      ...
      if (schedules.isEmpty()) {
         ...
         /*find the next execution date for the schedule and try to execute it*/
         while (now >= nextExecutionTime) {
            lastExecutedDate = new Date(nextExecutionTime);
            persistedSchedule.setLastExecuted(lastExecutedDate);
            nextExecutionTime = getNextExecutionTime(persistedSchedule);
            shouldProcess = true;
         }
      ...
      //timerFirstDelay = 10 * 1000; //only for testing
