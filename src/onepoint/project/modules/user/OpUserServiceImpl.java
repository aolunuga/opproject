...
//     };
   ...
   public void insertUser(OpProjectSession session, OpBroker broker, OpUser user)
      ...
      if ((!Boolean.valueOf(OpSettingsService.getService().getStringValue(session, OpSettings.ALLOW_EMPTY_PASSWORD))) &&
   ...
   private void authenticateUser(OpProjectSession session, OpBroker broker, OpUser user) {
      ...
      if (user_locale == null) {
         ...
         user_locale = XLocaleManager.findLocale(OpSettingsService.getService().getStringValue(session, OpSettings.USER_LOCALE_ID));
      }
