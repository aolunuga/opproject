...
public class OpSettingsModule extends OpModule {
   ...
   private static final XLog logger = XLogFactory.getLogger(OpSettingsModule.class);
   public static final String OLD_LOCALE_SETTING = "en";


   public void start(OpProjectSession session) {
      // Load settings
      OpSettingsService.getService().loadSettings(session, true);
   }
      ...
      try {
         ...
         while (result.hasNext()) {
            ...
            if (OpPreference.LOCALE_ID.equals(preference.getName())) {
               ...
               if ("en".equals(value)) {
                  logger.info("Changed preference value from " + value + " to " + "en_US");
         ...
         if (settings != null) {
            String settingsLocale = settings.getStringValue(OpSettings.USER_LOCALE_ID);
            if (OLD_LOCALE_SETTING.equals(settingsLocale)) {
               settings.putStringValue(OpSettings.USER_LOCALE_ID, newLocaleId);
            }
         }
