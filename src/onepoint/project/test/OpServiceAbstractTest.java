...
public abstract class OpServiceAbstractTest extends MockObjectTestCase {
   ...
   /**
    * Class that implements the Constraint interface in order to be used to verify the parameter passed to the find
    * method on a broker.
    */
   private class FindConstraint implements Constraint {
      ...
      /**
       * Method used to evaluate if the constraint has been respected or not.
       *
       * @param object Object being evaluated
       * @return true if the constraint has not been broken.
       */
      public boolean eval(Object object) {
         if (!(object instanceof OpPath)) {
            return false;
         }
         OpPath path = (OpPath) object;
         Iterator steps = path.steps();
         if (steps.hasNext()) {
            OpStep step = (OpStep) steps.next();
            if (!step.getNodeTest().equals(type)) {
               return false;
            }
            Iterator predicates = step.predicates();
            OpExpression predicate = (OpExpression) predicates.next();
            OpOperatorExpression query = (OpOperatorExpression) predicate.getRootExpression();
            OpVariableExpression var = (OpVariableExpression) query.getLeftOperand();
            if (!var.getVariable().equals(action)) {
               return false;
            }
            OpValueExpression val = (OpValueExpression) query.getRightOperand();
      ...
      public StringBuffer describeTo(StringBuffer stringBuffer) {
         return stringBuffer.append("Will determine if the given path to find is correct");
      }
   ...
   /**
    * Constructs a findConstraint based on the given 2 strings used for validating the passed value to find method
    * on a broker.
    *
    * @param type   - type of node expected to be found in the path expression passed to the broker (it is the type of
    *               the entity that is being searched
    * @param action type of action that is being used (e.g Login)
    * @param value  value to be tested (e.g name of the user for the Login command)
    * @return a new FindConstraint that will verify if the path given to find respects the type and user name.
    */
   protected Constraint evaluateXPathArg(String type, String action, String value) {
      FindConstraint findConstr = new FindConstraint();
      findConstr.setType(type);
      findConstr.setValue(value);
      findConstr.setAction(action);
      return findConstr;
   }
