...
public class OpProjectProgressFormProvider implements XFormProvider {
   ...
   private OpQuery getQueryWithoutBaseline(OpBroker broker, long projectId) {
      ...
      queryBuffer.append(" where project.Archived=false and project.id = ? and activity.OutlineLevel = 0 and activity.Deleted = false and activity.Type in (0, 1, 2)");
      queryBuffer.append(" order by activity.Sequence");
   ...
   private OpQuery getQueryWithBaseline(OpBroker broker, long projectId, long baselineId) {
      ...
      queryBuffer.append(" where project.Archived=false and project.id = ? and activity.OutlineLevel = 0 and activity.Deleted = false and activity.Type in (0, 1, 2) and planVersion.id = ?");
      queryBuffer.append(" order by activity.Sequence");
